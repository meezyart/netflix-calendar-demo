...
class Calendar extends React.Component {
  ...
  findRelease = (date) => {
    ...
    return this.state.releases.filter(node => m(node.launch_date).format('L') === formattedDate)
  }
  ...
  renderDay = (date, month) => {
    ...
    // console.log("day",  this.findRelease(date));
    let monthDiff = day.isCurrentMonth ? "" : " month-diff",
      today = day.isToday ? " is-today" : "",
      hasRelease = day.hasReleaseToday.length  > 0 ? " release-today" : "";
    ...
    return (
      ...
      >
        ...
        <div className="release">{(day.hasReleaseToday.length  > 0) ? day.hasReleaseToday[0].title:''  }</div>
      </div>
    );
  };
  renderWeek = (day, month) => {
    const days = [];
    let date = day;
    // console.log(date, "renderWeek", month, release)
    for (var i = 0; i < 7; i++) {
  ...
  renderMonth = () => {
    ...
    while (!finish) {
      // console.log(date, monthIndx);
      weeks.push(
        this.renderWeek(date, this.state.currentMonth, this.state.release)
      );
      date.add(1, "w");
  ...
  render() {
    return (
      <section className="calender-container">
        ...
        <footer><img src={Logo} alt="" width="150"/></footer>
      </section>
    );
  }
}
